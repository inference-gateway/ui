...
export async function POST(req: Request) {
  const isAuthEnabled = process.env.ENABLEAUTH === 'true';
              ...
              try {
                if (chunk.choices?.[0]?.delta?.content) {
                  ...
                  if (content.includes('<think>')) {
                    ...
                    if (!chunk.choices[0].delta.reasoning_content && thinkContent.trim()) {
                      chunk.choices[0].delta.reasoning_content = thinkContent.trim();
                    }
                    thinkContent = '';
                  } else if (inThinkTag) {
